"""
Closed-loop trajectory optimization / co-design with LQR control using Dymos
Control gain matrix is determined by solving the Riccati equation by a Newton solver.
Then, closed-loop trajectory is analyzed/optimized by Radau collocation method.

The results from this runscript will NOT appear in the journal paper (instead, use dynOpt's results). This is just a reference.
"""
...
from quadrotormodels.linearize_dynamics import LinearizeDynamics
from quadrotormodels.lqr import LQRFeedbackGain
...
if __name__ == '__main__':
    ...
    # --- LQR settings ---
    Q = np.eye(6)
    # Q[1, 1] = 0   # don't care the target altitude
    R = np.eye(2)
    ...
    if flag_opt_design:
        # with CCD, theta increases and chord decreases
    ...
    # linearize system at the trim state (= reference state)
    p.model.add_subsystem('linearize', LinearizeDynamics(params_dict=params_dict), promotes=['*'])
    p.model.connect('trim.omega_vert_1', 'omega_ref')   # note: assuming omega_vert_1 = omega_vert_2

    # solve LQR for feedback gain K
    p.model.add_subsystem('LQR', LQRFeedbackGain(Q=Q, Rinv=Rinv), promotes=['*'])

    # --- closed-loop simulation using Dymos ---
    ...
    phase.add_parameter('K', val=np.zeros((2, 6)), static_target=True, include_timeseries=False)   # control feedback matrix, shape (n_control, n_states)
    phase.add_parameter('omega_ref', val=330., units='rad/s', static_target=True)
    p.model.connect('K', 'traj.phase.parameters:K')
    p.model.connect('trim.omega_vert_1', 'traj.phase.parameters:omega_ref')   # assuming trim.omega_vert_1 = trim.omega_vert_2 -> omega_ref
    ...
    # (1) LQR objective
    phase.add_objective('cost', loc='final', ref=100)   # LQR objective
    ...
    if optimizer == 'SNOPT':
        ...
        p.driver.opt_settings['Function precision'] = 1e-7
    ...
    p.setup(check=True)
    om.n2(p, outfile='n2_lqr_pre.html', show_browser=False)
    # -------------------------------------
    
    # initial guess of P for Q = eye, R = eye (from Matlab lqr command)
    P0 = np.array([[2.3477, 0.0000, -11.9787, 2.2546, 0.0000, -3.2212],
                   [0.0000, 27.5506, -0.0000 , 0.0000, 34.0042, -0.0000],
                   [-11.9787, -0.0000, 190.8230, -24.8820, -0.0000, 71.2452],
                   [2.2546, 0.0000, -24.8820, 4.0695, 0.0000, -7.5553],
                   [0.0000, 34.0042, -0.0000, 0.0000, 43.6013, -0.0000],
                   [-3.2212, -0.0000, 71.2452, -7.5553, -0.0000, 38.5861]])
    # P0[:, 1] = 0   # if I don't penalize altitude y
    ...
    # --- setup recorder ---
    recorder = om.SqliteRecorder('opt_log_lqr.sql')
    ...
    p.setup(check=True)   # for some reason, I have to setup the recorder after calling dm.run_problem() for shooting. Then I need to setup the problem here again. 
    ...
    # run collocation optimization
    # p.check_partials(compact_print=True)
    p.run_driver()

    om.n2(p, outfile='n2_lqr_aft.html', show_browser=False)
    ...
    # --- print results ---
    print('omega_ref =', p.get_val('omega_ref'))
    print('A =', p.get_val('A'))
    print('B =', p.get_val('B'))
    print('K =', p.get_val('K'))
    print('trim power:', p.get_val('trim.power', units='W'), 'W')
    print('final LQR cost:', p.get_val('traj.phase.timeseries.input_values:states:cost')[-1, 0])
    print('final energy:', energy1, 'W*s (trapez integration)')
    print('--- designs ---')
    ...
    # print time histories
    # print('--------------------------------')
    # print('time histories')
    # print('time_init =', list(p.get_val('traj.phase.timeseries.time').reshape(100))[0:100:2])
    # print('x_init =', list(p.get_val('traj.phase.timeseries.states:x', units='m').reshape(100))[0:100:2])
    # print('y_init =', list(p.get_val('traj.phase.timeseries.states:y', units='m').reshape(100))[0:100:2])
    # print('theta_init =', list(p.get_val('traj.phase.timeseries.states:theta', units='deg').reshape(100))[0:100:2])
    # print('vx_init =', list(p.get_val('traj.phase.timeseries.states:vx', units='m/s').reshape(100))[0:100:2])
    # print('vy_init =', list(p.get_val('traj.phase.timeseries.states:vy', units='m/s').reshape(100))[0:100:2])
    # print('theta_vel_init =', list(p.get_val('traj.phase.timeseries.states:theta_vel', units='deg/s').reshape(100))[0:100:2])
    # print('cost_init =', list(p.get_val('traj.phase.timeseries.states:cost', units='s').reshape(100))[0:100:2])
    # print('energy_init =', list(p.get_val('traj.phase.timeseries.states:energy', units='W*s').reshape(100))[0:100:2])
    # print('--------------------------------')
